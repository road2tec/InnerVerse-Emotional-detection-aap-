    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (err.response) {
                ...
                msg = err.response.data?.detail || 'Incorrect email or password.';
            } else if (err.request) {
                // Request was made but no response received (Network error)
                msg = `Network Error: Cannot reach backend at ${BASE_URL}. Make sure backend is running on port 8000, phone and laptop are on same Wi-Fi, and mobile data (LTE) is off.`;
            } else {
                msg = err.message;
            }
