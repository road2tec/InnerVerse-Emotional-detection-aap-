...
export default function RecommendScreen({ navigation, route }) {
    ...
    const handleRecommend = async (overrideEmotion) => {
        ...
        try {
            ...
            // Determine age group from numeric age or use selected age group
            let finalAgeGroup = ageGroup;
            const numericAge = parseInt(age, 10);
            if (numericAge > 0) {
                // Convert numeric age to age group
                if (numericAge <= 12) finalAgeGroup = 'child';
                else if (numericAge <= 19) finalAgeGroup = 'teen';
                else if (numericAge <= 59) finalAgeGroup = 'adult';
                else finalAgeGroup = 'senior';
            }

            // Call API with correct parameters: (emotion, ageGroup, userId)
            const data = await getActivityRecommendations(emotion, finalAgeGroup, userId);
            setResult(data);
        } catch (err) {
            console.error('Recommendation error:', err);
            const msg = err?.response?.data?.detail || err?.response?.data?.message || 'Failed to load recommendations. Please check your connection.';
                ...
                {result && emotionInfo && (
                    <Animated.View style={[
                            ...
                            {result.ai_generated && (
                                <View style={[S.aiBadge, { borderColor: emotionInfo.color }]}>
                                    <Text style={[S.aiBadgeText, { color: emotionInfo.color }]}>
                                        ✨ AI {result.model_used || 'Generated'}
                                    </Text>
                                </View>
                            )}
                        </View>
                        ...
                        <Text style={S.activitiesLabel}>
                            Recommended Activities ({result.activities.length} activities)
                        </Text>
...
const S = StyleSheet.create({
    ...
    // AI Badge
    aiBadge: { borderRadius: 10, paddingHorizontal: 8, paddingVertical: 4, borderWidth: 1, backgroundColor: '#0A0A1A' },
