...
import { detectTextEmotion, getTextEmotionModelInfo } from '../services/textEmotionService';
...
export default function TextDetectScreen({ navigation }) {
    ...
    const [modelInfo, setModelInfo] = useState({ type: 'keyword-based', model_name: 'Loading...' });
    ...
    useEffect(() => {
        const loadModelInfo = async () => {
            try {
                const info = await getTextEmotionModelInfo();
                setModelInfo(info);
            } catch (err) {
                console.warn('Could not load model info:', err);
                setModelInfo({ type: 'keyword-based', model_name: 'Keyword Detection' });
            }
        };
        loadModelInfo();
    }, []);
                ...
                <View style={S.header}>
                    ...
                    <View style={S.headerContent}>
                        ...
                        <Text style={S.subtitle}>
                            {modelInfo.type === 'keyword-based'
                                ? 'Advanced Keyword-based Emotion Detection'
                                : 'BERT-based NLP Emotion Detection'
                            }
                        </Text>
                        <View style={S.modelBadge}>
                            <Text style={S.modelBadgeText}>
                                {modelInfo.type === 'keyword-based'
                                    ? '🔤 Keyword Detection · 7 Emotions'
                                    : `🤖 ${modelInfo.model_name} · 7 Emotions`
                                }
                            </Text>
                        </View>
                    </View>
