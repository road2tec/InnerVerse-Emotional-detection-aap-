...
import { detectVoiceEmotion, getVoiceModelInfo } from '../services/voiceEmotionService';
...
export default function VoiceDetectScreen({ navigation }) {
    ...
    const [modelInfo, setModelInfo] = useState({
        model_type: 'Rule-based acoustic classifier',
        model_name: 'Loading...'
    });
    ...
    // Load voice model information
    useEffect(() => {
        const loadModelInfo = async () => {
            try {
                const info = await getVoiceModelInfo();
                setModelInfo(info);
            } catch (err) {
                console.warn('Could not load voice model info:', err);
                setModelInfo({
                    model_type: 'Rule-based acoustic classifier',
                    model_name: 'Keyword Detection'
                });
            }
        };
        loadModelInfo();
    ...
    const handleAnalyse = async () => {
        ...
        try {
            ...
            const fullMsg = backendDetail || err?.message || String(err);
            Alert.alert('⚠️ Analysis Error', `Analysis failed. Detail: ${fullMsg}`);
        }
    };
                ...
                <View style={S.header}>
                    ...
                    <View style={S.headerContent}>
                        ...
                        <Text style={S.subtitle}>
                            {modelInfo.model_type === 'Keras custom (.h5)'
                        ...
                        <View style={S.modelBadge}>
                            <Text style={S.modelBadgeText}>
                                {modelInfo.model_type === 'Keras custom (.h5)'
                                    ? '🧠 Custom Voice Model · 5 Emotions'
                                    : modelInfo.model_type === 'Rule-based acoustic classifier'
                                        ? '🔊 Rule-based Audio · 5 Emotions'
                                        : `📊 ${modelInfo.model_type} · 5 Emotions`
                                }
                            </Text>
                        </View>
                    </View>
...
const S = StyleSheet.create({
    ...
    activityNumber: { color: '#FF6B35', fontSize: 16, fontWeight: '800', minWidth: 24 },
