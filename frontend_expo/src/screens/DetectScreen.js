...
import { detectEmotionFromText, detectEmotionFromFacial, detectEmotionFromVoice, getRecommendations } from '../services/emotionService';
import { getStoredUser } from '../services/authService';
import { getAgeGroup, AGE_GROUPS } from '../utils/emotionUtils';
    ...
    const handleDetect = async () => {
        try {
            ...
            if (method === 'text') {
                ...
                // In a real app, use ImagePicker or Camera
                Alert.alert(
                    'Camera',
                    'In Phase 2, this will open the camera to capture your facial expression. For now, switching to text detection.',
                );
                return;
            } else if (method === 'voice') {
                // In a real app, use AudioRecorderPlayer
                Alert.alert(
                    'Microphone',
                    'In Phase 2, this will record your voice for emotion detection. For now, switching to text detection.',
                );
                return;
            }
