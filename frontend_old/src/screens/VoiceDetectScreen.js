...
export default function VoiceDetectScreen({ navigation }) {
    ...
    const startRecording = async () => {
        ...
        if (!recorder) {
            // Simulate mode (library not installed)
            Alert.alert(
                '🎤 Demo Mode',
                'Install react-native-audio-recorder-player to enable real recording.\n\nPress "Simulate" to test with demo audio data.',
    ...
    const handleAnalyse = async () => {
        ...
        try {
            ...
            setRecState(REC_STATE.RECORDED);
            const msg = err?.response?.data?.detail || 'Analysis failed. Check your connection and backend.';
            Alert.alert('⚠️ Analysis Error', msg);
        }
    };
